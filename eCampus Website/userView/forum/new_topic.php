...
<div class="box">
	<div class="box_left">
    	<a href="index1.php">Subject List</a> &gt; <a href="list_topics.php?parent=<?php echo $id; ?>">Topic List</a> &gt; New Topic
    </div>
	<div class="clean"></div>
</div>
<?php
if(isset($_POST['message'], $_POST['title']) and $_POST['title']!='')
{
	...
	$topic_insert=$topic->insert(array("parent"=>$id,"tid"=>"$title$id","id"=>"","id2"=>"1","title"=>$title,"message"=>$message,"authorID"=>$_SESSION["uname"]));
	if($topic_insert == "1"){
		header('Location: http://localhost/ecampus/userview/forum/list_topics.php?parent='.$id.'');
	}
}
else
{
